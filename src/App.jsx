      ...
      <div className="min-h-screen bg-stone-50 text-stone-900 font-sans pb-20">
        <nav className="bg-white border-b px-6 py-4 flex flex-col gap-4 sticky top-0 z-50 shadow-sm">
          <div className="flex justify-between items-center">
            ...
            <button onClick={() => setView('order')} className="p-2 hover:bg-stone-100 rounded-full">
              <X size={20} />
            </button>
          </div>
          <div className="flex bg-stone-100 p-1 rounded-2xl">
            <button
              ...
              className={`flex-1 py-2.5 text-xs font-black rounded-xl transition-all ${adminSubView === 'orders' ? 'bg-white shadow-sm text-[#6F3E1E]' : 'text-stone-400'}`}
            >
              주문현황
            </button>
            <button
              onClick={() => setAdminSubView('settings')}
              className={`flex-1 py-2.5 text-xs font-black rounded-xl transition-all ${adminSubView === 'settings' ? 'bg-white shadow-sm text-[#6F3E1E]' : 'text-stone-400'}`}
            >
              상품설정
            </button>
          </div>
                ...
                <div key={order.id} className="bg-white rounded-[2rem] p-6 shadow-sm border border-stone-200">
                  <div className="flex justify-between items-start mb-4">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        {order.status === 'paid' && (
                          <span className="text-[10px] font-black text-green-600 bg-green-50 px-2 py-0.5 rounded-full">결제완료</span>
                        )}
                      </div>
                  ...
                  <div className="bg-stone-50 rounded-2xl p-4 space-y-2 text-sm">
                    {Object.entries(order.items || {}).map(([pid, qty]) => {
                      if (!qty) return null;
                      const product = settings.products.find(x => x.id === pid);
                      return (
                  ...
                  <button
                    ...
                    className="w-full mt-4 text-[10px] font-bold text-stone-300 hover:text-red-400 py-2"
                  >
                    주문 삭제
                  </button>
                </div>
            ...
            <div className="space-y-6">
              <div className="bg-white p-7 rounded-[2.5rem] border border-stone-200 shadow-sm">
                <div className="flex justify-between items-center mb-6 px-1">
                  ...
                  <button
                    onClick={() => {
                      const date = prompt('추가할 날짜 (예: 03/01 (일))');
                      if (date) {
                        setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'config', 'settings'), {
                          ...settings,
                          pickupDates: [...settings.pickupDates, date]
                        });
                      }
                    }}
                    className="w-10 h-10 bg-stone-100 hover:bg-[#6F3E1E] hover:text-white rounded-2xl flex items-center justify-center transition-all"
                  >
                    <Plus size={20} />
                  </button>
                </div>
                <div className="flex flex-wrap gap-2">
                  {settings.pickupDates.map(date => (
                    <span key={date} className="pl-4 pr-3 py-2 bg-stone-50 border border-stone-100 rounded-xl text-xs flex items-center gap-3 font-bold">
                      {date}
                      <X
                        size={14}
                        className="cursor-pointer text-stone-300 hover:text-red-500"
                        onClick={() =>
                          setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'config', 'settings'), {
                            ...settings,
                            pickupDates: settings.pickupDates.filter(x => x !== date)
                          })
                        }
                      />
                    </span>
              ...
              <div className="bg-white p-7 rounded-[2.5rem] border border-stone-200 shadow-sm">
                    ...
                    onClick={() => {
                      ...
                      if (name && originalPrice && price) {
                        setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'config', 'settings'), {
                          ...
                          products: [
                            ...settings.products,
                            { id: `p_${Date.now()}`, name, originalPrice: parseInt(originalPrice, 10), price: parseInt(price, 10) }
                          ]
                        });
                      }
                    }}
                ...
                <div className="space-y-3">
                  {settings.products.map(product => (
                    <div key={product.id} className="flex justify-between items-center p-5 bg-stone-50 rounded-2xl border border-stone-100">
                      <div>
                        <span className="text-sm font-black text-stone-800 block">{product.name}</span>
                        <div className="flex items-center gap-2 mt-1">
                          <span className="text-[10px] text-stone-300 line-through font-bold">{product.originalPrice?.toLocaleString()}원</span>
                          <span className="text-xs font-black text-[#6F3E1E]">{product.price?.toLocaleString()}원</span>
                        </div>
                      </div>
                      <button
                        onClick={() => {
                          if (confirm('삭제할까요?')) {
                            setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'config', 'settings'), {
                              ...settings,
                              products: settings.products.filter(x => x.id !== product.id)
                            });
                          }
                        }}
                        className="p-2 text-stone-200 hover:text-red-500"
                      >
                        <Trash2 size={18} />
                      </button>
                    </div>
      ...
      {!submitted ? (
          ...
          <form onSubmit={handleOrderSubmit} className="space-y-8">
            <div className="bg-white rounded-[3rem] p-10 shadow-2xl shadow-[#6F3E1E]/5 border border-stone-100 space-y-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-[#A88B73] uppercase tracking-[0.2em] flex items-center gap-2 ml-1">
                  <User size={12} /> Name
                </label>
                <input
                  required
                  type="text"
                  placeholder="성함"
                  className="w-full h-14 px-5 bg-stone-50 rounded-2xl font-black text-sm outline-none border-2 border-transparent focus:border-[#6F3E1E]/10 transition-all"
              ...
              <div className="space-y-2">
                ...
                <select
                  required
                  className="w-full h-14 px-5 bg-stone-50 rounded-2xl font-black text-sm outline-none border-2 border-transparent focus:border-[#6F3E1E]/10 transition-all appearance-none"
            ...
            <div className="bg-white rounded-[3rem] p-10 shadow-2xl shadow-[#6F3E1E]/5 border border-stone-100">
              <h3 className="text-xs font-black text-[#A88B73] uppercase tracking-[0.2em] mb-10 flex items-center gap-2">
                <ShoppingBasket size={18} /> Menu List
              </h3>
              <div className="space-y-10">
                {settings.products.map(product => (
                  <div key={product.id} className="flex justify-between items-center group">
                    <div className="space-y-1">
                      <p className="font-black text-base text-stone-800">{product.name}</p>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-black text-[#6F3E1E] bg-[#6F3E1E]/5 px-2 py-0.5 rounded-lg flex items-center gap-1">
                          {product.price.toLocaleString()}원 <Tag size={10} />
                        </span>
                      </div>
                    </div>
                    <div className="flex items-center gap-3 bg-stone-50 p-1 rounded-xl border border-stone-100">
                      <button
                        type="button"
                        onClick={() => {
                          const qty = orderForm.items[product.id] || 0;
                          if (qty > 0) {
                            setOrderForm({
                              ...orderForm,
                              items: { ...orderForm.items, [product.id]: qty - 1 }
                            });
                          }
                        }}
                        className="w-8 h-8 bg-white rounded-lg shadow-sm flex items-center justify-center font-black text-[#6F3E1E]"
                      >
                        -
                      </button>
                      <span className="w-4 text-center text-xs font-black">{orderForm.items[product.id] || 0}</span>
                      <button
                        type="button"
                        onClick={() => {
                          const qty = orderForm.items[product.id] || 0;
              ...
              <div className="max-w-md mx-auto flex items-center justify-between gap-4 bg-[#2D1B10] p-5 rounded-[2rem] shadow-2xl">
                <div className="flex flex-col pl-2">
                  ...
                  <span className="text-xl font-black text-white">
                    {Object.entries(orderForm.items).reduce((sum, [id, qty]) => {
                      const product = settings.products.find(x => x.id === id);
                      return sum + (product ? product.price * qty : 0);
                    }, 0).toLocaleString()}원
                  </span>
                ...
                <button
                  type="submit"
                  className="flex-1 h-14 bg-[#6F3E1E] text-white rounded-2xl font-black text-sm hover:brightness-110 active:scale-[0.98] transition-all flex items-center justify-center gap-2"
                >
        ...
        <div className="bg-white rounded-[4rem] p-16 text-center shadow-2xl border border-stone-100 space-y-4">
          ...
          <div className="text-sm text-stone-400 font-bold leading-relaxed space-y-1">
            ...
            <p className="text-sm text-stone-700">예금주 박지현</p>
          ...
          <button
            onClick={() => {
              setSubmitted(false);
              setOrderForm({ name: '', phone: '', pickupDate: '', items: {} });
            }}
            className="text-[#A88B73] font-black text-xs border-b border-[#A88B73]/20 pb-0.5"
          >
            신규 예약
          </button>
        </div>
